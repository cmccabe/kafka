...
class LinuxIoMetricsCollectorTest extends Logging {
  ...
  class TestDirectory() {
    ...
    def writeProcFile(readBytes: Long, writeBytes: Long, cancelledWritesBytes: Long) = {
      ...
      bld.append("cancelled_write_bytes: %d%n".format(cancelledWritesBytes))
  ...
  def testReadProcFile(): Unit = {
    ...
    testDirectory.writeProcFile(123L, 456L, 0L)
    ...
    testDirectory.writeProcFile(124L, 457L, 0L)
    assertEquals(123L, collector.readBytes())
    assertEquals(456L, collector.writeBytes())
    ...
    // Test cancelled write bytes
    testDirectory.writeProcFile(124L, 457L, 100L)
    time.sleep(1)
    assertEquals(124L, collector.readBytes())
    assertEquals(357L, collector.writeBytes())
  }
