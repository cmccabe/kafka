...
import org.apache.kafka.image.{ClusterImage, ImageProvenance, MetadataDelta, MetadataImage}
    ...
    cache match {
      ...
      case c: KRaftMetadataCache => {
        ...
        request.topicStates().forEach { topic =>
          toRecords(topic).foreach(delta.replay(_))
        }
        c.setImage(delta.apply(ImageProvenance.EMPTY))
      }
...
class MetadataCacheTest {
  ...
  def testIsBrokerFenced(): Unit = {
    ...
    metadataCache.setImage(delta.apply(ImageProvenance.EMPTY))
