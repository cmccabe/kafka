...
class BrokerMetadataSnapshotter(
  ...
  override def maybeStartSnapshot(image: MetadataImage, snapshotReasons: Set[SnapshotReason]): Boolean = synchronized {
    if (_currentSnapshotOffset != -1) {
      info(s"Declining to create a new snapshot at ${image.provenance().highestOffset()} because " +
        s"there is already a snapshot in progress at offset ${_currentSnapshotOffset}")
      false
    } else {
      val writer = writerBuilder.build(
        image.provenance().highestOffset(),
        image.provenance().highestEpoch(),
        image.provenance().highestTimestamp()
      )
      if (writer.nonEmpty) {
        _currentSnapshotOffset = image.provenance().highestOffset()
        ...
        info(s"Declining to create a new snapshot at ${image.provenance().highestOffset()} because " +
          s"there is already a snapshot at offset ${image.provenance().highestOffset()}")
        false
      }
    }
  }
