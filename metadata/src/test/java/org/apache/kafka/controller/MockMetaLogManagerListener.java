...
public class MockMetaLogManagerListener implements MetaLogManager.Listener {
    ...
    @Override
    public void commit(long epoch, long index, ApiMessage message) {
        try {
            commitHandler.handle(nodeId, epoch, index, message);
        } catch (Exception e) {
            firstError.compareAndSet("", "error handling commit(epoch=" + epoch +
    ...
    @Override
    public synchronized void claim(long epoch) {
        if (epoch < 0) {
            firstError.compareAndSet("", nodeId + " invalid negative epoch in claim(" +
                epoch + ")");
            return;
        }
        if (shutdown) {
            firstError.compareAndSet("", nodeId + " invoked claim(" + epoch + ") after " +
                "shutdown.");
            return;
        }
        if (curEpoch != -1L) {
            firstError.compareAndSet("", nodeId + " invoked claim(" + epoch + ") before " +
    ...
    @Override
    public synchronized void renounce(long epoch) {
        if (curEpoch != epoch) {
            firstError.compareAndSet("", nodeId + " invoked renounce(" + epoch + ") " +
