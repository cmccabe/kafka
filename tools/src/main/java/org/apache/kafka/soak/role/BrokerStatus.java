...
import static org.apache.kafka.soak.role.RoleDependencies.BROKER_STATUS;

/**
 * Gets the status of the broker.
 */
public final class BrokerStatus extends Action {
    private final RoleStatusCollector statusCollector;

    public BrokerStatus(String nodeName, RoleStatusCollector statusCollector) {
        super(BROKER_STATUS,
            nodeName,
            new String[] {},
            new String[] {BROKER_STATUS});
    ...
    public void call(SoakCluster cluster, SoakNode node) throws Throwable {
        statusCollector.collect(node.nodeName(), SoakUtil.
                getJavaProcessStatus(cluster, node, KAFKA_CLASS_NAME));
    }
}
