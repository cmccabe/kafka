...
import static org.apache.kafka.soak.role.RolePaths.TROGDOR_CONF_SUFFIX;
import static org.apache.kafka.soak.role.RolePaths.TROGDOR_START_SCRIPT;

public class TrogdorStart extends Action  {
    private final TrogdorDaemonType daemonType;

    public TrogdorStart(String nodeName, TrogdorDaemonType daemonType) {
        super(daemonType.start(),
            nodeName,
            new String[] {"?" +LINUX_SETUP + ":" + nodeName,
                RSYNC_SRC + ":" + nodeName},
            new String[] {daemonType.start()});
    ...
    public static String[] createSetupPathsCommandLine(TrogdorDaemonType daemonType) {
        return new String[] {"-n", "--", "rm", "-f", daemonType.logPath(), "&&",
            "sudo", "mkdir", "-p", daemonType.root(), RolePaths.LOGS_ROOT, "&&",
    ...
    private File writeTrogdorConfig(SoakCluster cluster, SoakNode node) throws IOException {
        ...
        try {
            file = new File(cluster.logPath(), String.format("trogdor-%s-%d.conf",
