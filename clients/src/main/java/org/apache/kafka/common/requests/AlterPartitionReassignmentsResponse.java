...
public class AlterPartitionReassignmentsResponse extends AbstractResponse {
    ...
    public AlterPartitionReassignmentsResponse(Struct struct) {
        short latestVersion = (short) (AlterPartitionReassignmentsResponseData.SCHEMAS.length - 1);
        this.data = new AlterPartitionReassignmentsResponseData(struct, latestVersion);
    }
    ...
    public static AlterPartitionReassignmentsResponse parse(ByteBuffer buffer, short version) {
        return new AlterPartitionReassignmentsResponse(
                ApiKeys.ALTER_PARTITION_REASSIGNMENTS.responseSchema(version).read(buffer), version);
    }
