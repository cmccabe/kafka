...
public class KafkaAdminClient extends AdminClient {
    ...
    /**
     * The selector to close when closing the client, or null if there is none.
     */
     ...
     */
    private long getDeadlineMs(int optionTimeoutMs) {
        if (optionTimeoutMs >= 0)
            return time.milliseconds() + optionTimeoutMs;
    ...
    static void closeQuietly(String what, AutoCloseable closeable) {
        ...
        try {
            ...
            log.error("Error closing " + what, exc);
        }
    }
    ...
    static KafkaAdminClient create(AdminClientConfig config) {
        ...
        try {
            ...
            networkClient = new NetworkClient(selector,
                metadata,
                clientId,
                100,
                ...
                apiVersions);
            channelBuilder = null;
            return new KafkaAdminClient(config, clientId, time, metadata, metrics, networkClient, selector);
        } catch (Throwable exc) {
    ...
    private KafkaAdminClient(AdminClientConfig config, String clientId, Time time, Metadata metadata,
                     Metrics metrics, KafkaClient client, Selector selector) {
    ...
    private abstract class Call {
        ...
        final void fail(long now, Throwable throwable) {
            ...
            tries++;
            if ((tries > MAX_CALL_RETRIES) || getTimeoutMsRemainingAsInt(now, deadlineMs) < 0) {
    ...
    private final class AdminClientRunnable implements Runnable {
        ...
        private int timeoutCalls(long now, Collection<Call> calls) {
            ...
            for (Iterator<Call> iter = calls.iterator(); iter.hasNext(); ) {
                Call call = iter.next();
                if (getTimeoutMsRemainingAsInt(now, call.deadlineMs) >= 0) {
                    break;
                }
        ...
        private long sendEligibleRpcs(long now, Map<Node, List<Call>> callsToSend,
            ...
            for (Iterator<Map.Entry<Node, List<Call>>> iter = callsToSend.entrySet().iterator();
                ...
                int timeoutMs = getTimeoutMsRemainingAsInt(now, call.deadlineMs);
        ...
        **/
        ...
        public void run() {
            ...
            closeQuietly("KafkaClient", client);
            closeQuietly("Metrics", metrics);
            closeQuietly("Selector", selector);
    ...
    public CreateTopicResults createTopics(final Collection<NewTopic> newTopics,
        ...
        runnable.call(new Call("createTopics", getDeadlineMs(options.timeoutMs()),
        ...
        runnable.call(new Call("listTopics", getDeadlineMs(options.timeoutMs()), new LeastLoadedNodeProvider()) {
            ...
            void handleResponse(AbstractResponse abstractResponse) {
                ...
                for (String topicName : cluster.topics()) {
                    topicListing.put(topicName, new TopicListing(topicName, cluster.internalTopics().contains(topicName)));
                }
